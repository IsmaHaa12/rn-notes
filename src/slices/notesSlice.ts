...
const initialState: NotesState = { items: {}, order: [] };
...
export const selectAllNotes = createSelector([selectNotesDomain], (d) => d.order.map((id) => d.items[id]));
export const selectPinnedNotes = createSelector([selectAllNotes], (notes) => notes.filter((n) => n.pinned));
export const selectNoteById = (id: string) => createSelector([(s: RootState) => s.notes], (d) => d.items[id]);
